...
class JInstallationHelper
{
	...
	function backupDatabase(& $db, $DBname, $DBPrefix, & $errors)
	{
		if($DBPrefix == '')
		{
			return; // Do nothing for empty prefix databases
		}
	...
	function deleteDatabase(& $db, $DBname, $DBPrefix, & $errors)
	{
		if($DBPrefix == '')
		{
			return 0; // Do nothing for empty prefix databases
		}
	...
	function createAdminUser(& $vars)
	{
		...
		if($adminPassword == '') return; // No password == keep password
		...
		$query = "SELECT * FROM #__users WHERE ".$db->nameQuote('id')." = 62";
		$db->setQuery($query);
		$result = $db->loadAssoc();
		if(!isset($result['username']))
		{
			// No user exists. Hmmm.... Let's create it, then
			$username = 'admin';
		}
		else
		{
			// The user already exists. Let's kill him and create a new one!
			$username = $result['username'];
			$query = "DELETE FROM #__users WHERE ".$db->nameQuote('id')." = 62";
			$db->setQuery($query);
			$db->query();
		}
		...
		$query = "INSERT INTO #__users VALUES (62, 'Administrator', ".$db->Quote($username).", ".$db->Quote($adminEmail).", ".$db->Quote($cryptpass).", 'Super Administrator', 0, 1, 25, '$installdate', '$nullDate', '', '')";
		$db->setQuery($query);
		if (!$db->query())
		{
